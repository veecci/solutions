...
int * _false;			/* clauses which are false */
int * lowfalse;
...
int _false[MAXCLAUSE];		/* clauses which are false */
        ...
        for (int iter = 0; iter < 100; ++iter) {
            ...
            for (int i = 0; i < numclause; ++i) {
                for (int j = 0; j < size[i]; ++j) {
                    int t = clause[i][j];
                    if      (t < 0 && !atom[t]) v[-t].push_back(i);
                    else if (t > 0 &&  atom[t]) v[ t].push_back(i);
                }
            }
            vid.clear();
            for (int i = 1; i <= numatom; ++i) vid.push_back(i);
            ...
            for (int i = 0; i < vid.size(); ++i) {
                ...
                for (int j = 0; j < v[t].size(); ++j) {
                    int cid = v[t][j];
                    bool flag_t = false;
                    for (int k = 1; k <= numatom; ++k) if (t != k) {
                        if (find(v[k].begin(), v[k].end(), cid) != v[k].end()) {
                            flag_t = true;
                            break;
                        }
                    }
            ...
            int zero_cnt = 0;
            for (int i = 1; i <= numatom; ++i) zero_cnt += v[i].empty();
            ret += zero_cnt;
        //printf("%d\n", zero_cnt);
        }
        printf("%.3lf\n", ret / 100.);
...
void initprob(void)
{
    ...
    _false = (int *) malloc(sizeof(int)*(numclause+1));
...
void flipatom(int toflip)
{
    ...
    for(i = 0; i < numocc; i++)
	  ...
	  if (++numtruelit[cli] == 1){
	      numfalse--;
	      _false[wherefalse[cli]] =
		_false[numfalse];
	      wherefalse[_false[numfalse]] =
		wherefalse[cli];
